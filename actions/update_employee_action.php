...
if ($_SERVER['REQUEST_METHOD'] == 'POST') 
{
    ...
    if(isset($_FILES['image']) && $_FILES['image']['error'] != UPLOAD_ERR_NO_FILE)
    {
            ...
            if (in_array($fileextenstion, $extensions) && $filesize < 2000000)
            {
                ...
                {   
                    ...
                    $update = update_employee_wimgctr($username, $useremail, $filepath, $employee_id);
                    if ($update)
                    {
                        // redirect back to the profile page
                        header("Location:../view/employee_profile_view.php");
                        exit();
                    }
                    else
                    {
    ...
    else 
    {
        ...
        $update = update_employee_ctr( $username, $useremail, $employee_id);
        if ($update)
        {
            // redirect back to the profile page
            header("Location:../view/employee_profile_view.php");
